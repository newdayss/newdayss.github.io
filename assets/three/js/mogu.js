$(function() {
	...
	$(document).click(function() {
		console.log(1);
		search_list.css('display', 'none'); //搜索列表功能	
	})
	...
	function bannerAction() {
		off = false;
		if(bannerNum == banner_list.length) {
			bannerNum = 0
		}
	...
	var endTime = new Date("2017/08/07 23:00:27");
	...
	var twoendTime = new Date("2017/08/07 22:10:00");
	...
	function toLastGoods() {
		document.body.onscroll = function() {
			var lastele = $('.needwidthbox>.goods_item').last();
			var lasteleOffHeight = (lastele.offset().top) - (lastele.height());
			if(document.body.scrollTop >= lasteleOffHeight && Loadingtimes < 4) {
				newItem();
				setTimeout(function() {
					Loadingtimes++;
					toLastGoods();
					lasteleOffHeight = (lastele.offset().top) - (lastele.height());
				}, 15)
